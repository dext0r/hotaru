...
void HDMICEC::stop() { pin_->detach_interrupt(); }
...
void HDMICEC::loop() {
  while (!recv_queue_.empty()) {
    ...
    ESP_LOGD(TAG, "[%d] frame received: %s", micros(), frame_str.c_str());
...
bool IRAM_ATTR HDMICEC::send_and_read_ack_(bool is_broadcast) {
  ...
  // normal messages: line pulled low by the target follower => message ACKed successfully. we need to flip the value to
  // match that logic.
  return (!value);
}

void IRAM_ATTR HDMICEC::switch_to_listen_mode_() { pin_->pin_mode(INPUT_MODE_FLAGS); }
